...
А) Выполнить `git fetch`, переключиться в `main`, сделать `git pull`, затем вернуться в `feature/notifications` и выполнить `git merge main`  
...
Г) Выполнить `git merge origin/main` в ветке `feature/notifications`, чтобы создать merge-коммит без предварительного fetch
...
Г) Выполнить `git reset --hard HEAD~1`, чтобы откатить последний коммит
...
В) Создать персональный токен в настройках GitLab
Г) Проверить соединение командой `ssh -T git@git.culab.ru`
...
А) Повторить POST-запрос, чтобы убедиться, что заказ создан  
...
Г) Игнорировать заголовок `Location` и считать, что заказ создан без дополнительных действий
...
А) GET `/api/tasks` — использовать GET для получения коллекции ресурсов  
Б) GET `/api/tasks/list` — использовать GET с явным указанием действия в пути  
В) POST `/api/tasks` с телом `{"action": "list"}` — использовать POST для получения списка  
Г) PUT `/api/tasks` — использовать PUT для получения списка
...
А) Логи сервера на наличие исключений и ошибок выполнения  
Б) Корректность тела запроса и заголовков, которые могли вызвать ошибку на сервере  
В) Наличие временных ограничений или таймаутов, которые могли привести к ошибке  
Г) Попробовать отправить тот же запрос через другой HTTP-метод, чтобы обойти ошибку
...
Г) Изменить HTTP-метод на GET, чтобы обойти ошибку
...
Б) Упрощает переиспользование бизнес-логики в разных частях приложения  
...
Б) `handlers/commands.py` — команды, `handlers/messages.py` — текст, `handlers/api.py` — интеграции, `handlers/storage.py` — данные  
В) `bot.py` — команды и текст, `external.py` — интеграции и хранилище  
Г) `main.py` — команды, `utils.py` — интеграции и хранилище
