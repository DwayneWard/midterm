...
Г) Выполнить `git fetch`, переключиться в `main`, сделать `git pull`, затем вернуться в `feature/notifications` и выполнить `git merge main`
Б) Выполнить `git fetch`, остаться в `feature/notifications` и выполнить `git rebase origin/main`, чтобы переписать коммиты поверх актуальной истории `main`
В) Выполнить `git pull` в ветке `feature/notifications`, чтобы автоматически объединить изменения через merge-коммит
...
В) Разрешить конфликты вручную и выполнить `git add settings.py`
Б) Вызвать `git merge --abort`, если нужно откатить merge
Г) Сделать `git reset --hard origin/main` и продолжить merge
...
А) Удалить файл из индекса командой `git rm --cached secrets.env`, добавить `secrets.env` в `.gitignore`, закоммитить изменения
Г) Удалить файл из рабочей директории командой `rm secrets.env`, перезапустить Git
В) Использовать `git stash` для временного сохранения файла, затем `git clean`
Б) Выполнить `git reset --hard HEAD~1`, чтобы откатить последний коммит

**Правильный ответ:** А### Вопрос 4
В GitLab нужно настроить репозиторий для работы по SSH с нового ноутбука. Какие шаги обязательны?
В) Сгенерировать пару ключей `ssh-keygen`
Г) Добавить публичный ключ в профиль GitLab
Б) Создать персональный токен в настройках GitLab
А) Проверить соединение командой `ssh -T git@git.culab.ru`
**Правильные ответы:** А, Б, Г
...
Г) Повторить POST-запрос, чтобы убедиться, что заказ создан
Б) Извлечь URL из заголовка `Location` и использовать его для получения или обновления созданного ресурса
А) Отправить DELETE-запрос на URL из заголовка `Location`, чтобы удалить созданный заказ
В) Игнорировать заголовок `Location` и считать, что заказ создан без дополнительных действий

**Правильный ответ:** Б### Вопрос 7
...
Г) GET `/api/tasks` — использовать GET для получения коллекции ресурсов
Б) GET `/api/tasks/list` — использовать GET с явным указанием действия в пути
А) POST `/api/tasks` с телом `{"action": "list"}` — использовать POST для получения списка
В) PUT `/api/tasks` — использовать PUT для получения списка

**Правильный ответ:** Г### Вопрос 8
...
А) Корректность тела запроса и заголовков, которые могли вызвать ошибку на сервере
Г) Наличие временных ограничений или таймаутов, которые могли привести к ошибке
В) Попробовать отправить тот же запрос через другой HTTP-метод, чтобы обойти ошибку
**Правильные ответы:** А, Б, В
...
А) `requests.post(url, files={"report": open("report.csv", "rb")}, data={"email": user})` — использовать `files` для файла и `data` для текстовых полей, библиотека автоматически установит правильный заголовок
Б) `requests.post(url, data={"report": open("report.csv", "rb"), "email": user})` — использовать только `data` для файла и текстовых полей, библиотека автоматически определит формат
В) `requests.post(url, json={"report": open("report.csv", "rb"), "email": user})` — использовать `json` для файла и текстовых полей, библиотека автоматически сериализует данные
...
В) Модули загрузятся дважды в разных контекстах, но программа выполнится
Г) Можно переместить импорт внутрь функции, где он используется, чтобы отложить загрузку модуля

**Правильный ответ:** А, Г### Вопрос 14
...
Б) Улучшает читаемость и навигацию по проекту
В) Позволяет переиспользовать модули в других частях приложения
Г) Уменьшает риск конфликтов имён между модулями из разных пакетов
А) Упрощает процесс компиляции Python-кода в байт-код
**Правильные ответы:** А, Б, В
...
**Правильный ответ:** Б, В### Вопрос 16
...
Г) Упрощает переиспользование бизнес-логики в разных частях приложения
В) Упрощает поддержку кода благодаря разделению ответственности между слоями
А) Ускоряет выполнение программы за счёт оптимизации интерпретатором
**Правильные ответы:** А, Б, В
...
А) Получить токен
В) Настроить описание бота и список команд
**Правильные ответы:** А, Б, В
...
Г) Вызвать `bot.polling()` для запуска бота
В) Настроить webhook и указать URL сервера
Б) Зарегистрировать обработчики команд
А) Отправить тестовое сообщение боту в Telegram

**Правильный ответ:** Г### Вопрос 19
Бот отвечает на команды, но не реагирует на текстовые сообщения. Что проверишь первым?
А) Зарегистрирован ли обработчик для текстовых сообщений
...
В) `bot/handlers.py` — команды, `services/api_client.py` — интеграции, `repositories/storage.py` — работа с данными
Г) `handlers/commands.py` — команды, `handlers/messages.py` — текст, `handlers/api.py` — интеграции, `handlers/storage.py` — данные
А) `bot.py` — команды и текст, `external.py` — интеграции и хранилище
Б) `main.py` — команды, `utils.py` — интеграции и хранилище

**Правильный ответ:** В### Вопрос 21
...
Б) Держать словарь `state[user_id] = "CONFIRM"` для отслеживания шага каждого пользователя
А) Полагаться на историю сообщений в чате и анализировать их порядок
