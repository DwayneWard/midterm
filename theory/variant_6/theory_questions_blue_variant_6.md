...
А) Выполнить `git fetch`, переключиться в `main`, сделать `git pull`, затем вернуться в `feature/inventory` и выполнить `git merge main`  
Б) Выполнить `git fetch`, остаться в `feature/inventory` и выполнить `git rebase origin/main`, чтобы переписать коммиты поверх актуальной истории `main`  
В) Выполнить `git pull` в ветке `feature/inventory`, чтобы автоматически объединить изменения через merge-коммит  
Г) Выполнить `git merge origin/main` в ветке `feature/inventory`, чтобы создать merge-коммит без предварительного fetch
...
А) Удалить файл из индекса командой `git rm --cached config.local.py`, добавить `config.local.py` в `.gitignore`, закоммитить изменения  
...
Б) Серия осмысленных коммитов с понятными сообщениями, каждое описывает отдельную логическую часть изменений  
В) Коммиты без сообщений, но с хорошим кодом и подробным описанием в README  
Г) Все изменения в одном коммите с сообщением «обновления»
...
А) Сразу считать операцию завершённой без дополнительных действий  
Б) Использовать URL из заголовка `Location` для получения или обновления созданного ресурса  
...
Г) HTTP-метод исходного запроса
...
А) PATCH `/api/users/{id}` с телом `{"phone": "+7..."}` — использовать PATCH для частичного обновления ресурса  
Б) GET `/api/users/{id}` с телом запроса — использовать GET для обновления  
В) POST `/api/users/phone` с телом `{"id": 123, "phone": "+7..."}` — использовать POST для обновления  
Г) PUT `/api/users/{id}` с телом, содержащим только поле phone — использовать PUT для частичного обновления
...
А) Проверить корректность формата JSON в теле запроса  
Б) Убедиться, что все обязательные поля присутствуют в теле запроса  
В) Проверить правильность типов данных для каждого поля  
...
А) Path-параметры задают идентификаторы ресурса и входят в структуру маршрута (например, `/users/{id}`)  
Б) Query-параметры используются для фильтрации, сортировки и передаются после символа `?` в URL (например, `/users?status=active`)  
В) Path-параметры определяют структуру эндпоинта и порядок их следования важен для маршрутизации  
Г) Query-параметры определяют структуру эндпоинта и порядок их следования важен для маршрутизации
...
В) Обернуть запрос в try/except и вернуть значение по умолчанию (например, пустой список) при ошибке соединения  
...
Б) Ошибка на стороне сервера; проверить корректность тела запроса и заголовков, которые могли вызвать ошибку на сервере  
В) Ошибка на стороне сервера; проверить наличие временных ограничений или таймаутов, которые могли привести к ошибке  
Г) Ошибка на стороне клиента; нужно исправить формат запроса и повторить его
...
Б) Перенести локальные импорты внутрь функций, чтобы отложить загрузку модулей  
...
Б) Использовать alias при импорте стандартного модуля: `import json as std_json`  
...
А) Упрощает тестирование отдельных слоёв без необходимости поднимать всю систему  
Б) Ускоряет процесс разработки за счёт параллельной работы над разными пакетами  
В) Уменьшает связанность между компонентами и улучшает читаемость кода  
Г) Упрощает процесс развёртывания, так как можно обновлять отдельные пакеты независимо
...
В) `from helpers import get_data`
Г) `import get_data`
...
В) Запущен ли бот
Г) Корректно ли указан токен бота
...
А) Использовать `InlineKeyboardMarkup` и добавить кнопки 
Б) Использовать `ReplyKeyboardMarkup` и добавить кнопки
В) Создать два отдельных обработчика для текстовых сообщений «Да» и «Нет»  
