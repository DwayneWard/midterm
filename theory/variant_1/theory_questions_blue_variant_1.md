...
А) Выполнить `git pull` в ветке `feature/auth`, чтобы автоматически объединить изменения через merge-коммит
Б) Выполнить `git merge origin/main` в ветке `feature/auth`, чтобы создать merge-коммит без предварительного fetch
В) Выполнить `git fetch`, остаться в `feature/auth` и выполнить `git rebase origin/main`, чтобы переписать коммиты поверх актуальной истории `main`
Г) Выполнить `git fetch`, переключиться в `main`, сделать `git pull`, затем вернуться в `feature/auth` и выполнить `git merge main`
...
А) Каждый разработчик клонирует репозиторий, создаёт отдельную ветку под свою задачу, коммитит изменения в этой ветке и пушит её в GitLab; когда задача готова, открывает Merge Request из своей ветки в `main`
Б) Каждый разработчик клонирует репозиторий, создаёт отдельную ветку под свою задачу, работает локально без пуша в GitLab; когда задача готова, делает один коммит, пушит ветку и открывает Merge Request из своей ветки в `main`
В) Каждый разработчик клонирует репозиторий, делает локальные коммиты в `main` по своей задаче и периодически пушит их в GitLab
...
А) Правило работает только если `.gitignore` расположен в папке `__pycache__`
Б) Они были закоммичены до добавления правила в `.gitignore`
В) `git status` игнорирует изменения правил `.gitignore`
Г) `.gitignore` не поддерживает маски по расширению
...
А) Сервер обязательно вернёт 405 Method Not Allowed для такого запроса
Б) Сервер может ответить, но это нарушает REST: обновление следует выполнять через PUT или PATCH
В) Сервер автоматически преобразует запрос в PUT перед обработкой
Г) POST подходит для любой операции, сервер просто выполнит обновление записи
...
А) Запрос обработан успешно; можно продолжать работу с полученными данными
Б) Ошибка на стороне сервера; сервер временно недоступен, нужно повторить запрос позже или обработать ошибку в коде
В) Ошибка на стороне клиента; нужно исправить запрос и повторить его
...
Б) POST `/api/books/get` с телом `{"id": 42}` — использовать POST для получения данных
В) PUT `/api/books/42` — использовать PUT для получения данных
Г) GET `/api/books?id=42` — использовать GET с идентификатором в query-параметре
...
А) Каждый запрос должен содержать всю необходимую информацию, сервер не хранит состояние клиента
Б) Хранить состояние сессии пользователя на сервере между запросами
В) Ответы сервера должны содержать информацию о возможности кеширования
Г) Использовать стандартные HTTP-методы (GET, POST, PUT, DELETE) для операций над ресурсами
...
А) Выбросится исключение `requests.Timeout`
Б) Запрос продолжит ждать ответа
В) Функция вернёт `None` без исключений
Г) Запрос автоматически повторится
...
А) Библиотека `requests` не поддерживает JSON
Б) Сервер вернул HTML или другой формат вместо JSON
В) JSON содержит синтаксическую ошибку
Г) Тело ответа пустое
...
А) Использовать параметр `json`: `requests.get("https://api.example.com/search", json={"q": "python", "limit": 10})`
Б) Вручную сформировать URL: `requests.get("https://api.example.com/search?q=python&limit=10")`
В) Использовать параметр `data`: `requests.get("https://api.example.com/search", data={"q": "python", "limit": 10})`
Г) Использовать параметр `params`: `requests.get("https://api.example.com/search", params={"q": "python", "limit": 10})`
...
А) Функция `send_mail` содержит синтаксическую ошибку
Б) Появился циклический импорт, из-за чего модуль `notifications` не успевает загрузиться полностью
В) Функция определена ниже импорта внутри `notifications`, поэтому Python её «не видит»
Г) Модуль `notifications` находится в другой директории и не найден при импорте
...
А) Python требует модульной структуры для запуска программ, без неё код не выполнится
Б) Изменения в одном модуле минимально влияют на другие, что упрощает тестирование и поддержку
В) Модули автоматически оптимизируются компилятором Python для ускорения выполнения
...
А) В `main.py` импортировать только `database`, так как тот уже тянет `api_client`
Б) В `database.py` импортировать `api_client`, а в `main.py` импортировать оба модуля
В) Импортировать все модули в `main.py`, чтобы избежать проблем с зависимостями
Г) Вынести общие зависимости в `config.py`
...
А) Упрощает добавление нового функционала без риска сломать существующий код
Б) Упрощает поиск и исправление ошибок благодаря изоляции модулей
В) Ускоряет выполнение программы за счёт оптимизации компилятором
...
А) Telegram будет отклонять все вызовы и бот перестанет отвечать пользователям
Б) Сможет отвечать только администратору, а остальные получат отказ
В) Бот перейдёт в тестовый режим с урезанным функционалом
Г) Telegram автоматически выдаст новый токен при следующем запросе
...
А) Telegram передаёт боту callback-запрос с `callback_data`, и зарегистрированный хендлер должен обработать событие
Б) Telegram отправляет обычное текстовое сообщение с текстом кнопки в чат
В) Telegram автоматически обрабатывает нажатие и отправляет пользователю предустановленное сообщение
