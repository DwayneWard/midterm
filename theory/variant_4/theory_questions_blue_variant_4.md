...
А) Выполнить `git fetch`, остаться в `feature/billing` и выполнить `git rebase origin/main`, чтобы переписать коммиты поверх актуальной истории `main`
Б) Выполнить `git pull` в ветке `feature/billing`, чтобы автоматически объединить изменения через merge-коммит
В) Выполнить `git fetch`, переключиться в `main`, сделать `git pull`, затем вернуться в `feature/billing` и выполнить `git merge main`
...
В) Сгенерировать пару ключей командой `ssh-keygen`, добавить приватный ключ в профиль GitLab, проверить соединение командой `git push`
Г) Сгенерировать пару ключей командой `ssh-keygen`, добавить публичный ключ в профиль GitLab, убедиться что ключ загружен в `ssh-agent`, проверить соединение командой `ssh -T git@gitlab.com`
...
А) Нужно сменить имя ветки; переименовать локальную ветку и повторить push
Б) Репозиторий повреждён; пересоздать репозиторий заново
В) GitLab отключил доступ; проверить настройки SSH-ключей или токенов в профиле
Г) Удалённая ветка содержит новые коммиты; выполнить `git pull` или `git fetch` с последующим merge/rebase, разрешить конфликты и повторить push
...
Б) Проверить корректность формата JSON в теле запроса и наличие всех обязательных полей
В) Проверить корректность URL и параметров пути запроса
Г) Проверить правильность типов данных для каждого поля в теле запроса
...
Б) GET `/api/orders/{order_id}` — использовать GET для получения ресурса по идентификатору в пути
В) GET `/api/orders/get?id={order_id}` — использовать GET с идентификатором в query-параметре
...
Б) Ошибка на стороне клиента; нужно исправить формат запроса и повторить его немедленно
В) Ошибка на стороне сервера; сервер временно недоступен, перегружен или на техническом обслуживании — нужно повторить запрос позже или обработать ошибку в коде
...
А) Модули загрузятся дважды, что приведёт к дублированию кода в памяти, но программа продолжит работу
...
Г) Модули могут загрузиться частично и вызывать `ImportError` при попытке использовать функции из неполностью загруженного модуля
...
А) Переместить локальный модуль в подпапку и использовать абсолютный импорт для стандартного модуля с alias: `import email as std_email`
...
Г) Переименовать свой файл, чтобы не совпадал с системным модулем
...
Б) Выбрать режим запуска в настройках телеграм бота
В) Получить токен
Г) /newbot
...
Б) Зарегистрировать три разных обработчика - сначала для команд `/start` и `/help`, затем для текста
В) Зарегистрировать обработчики - сначала для текста, затем для команд
Г) Обработчик только на `/help` и общий обработчик для остальных сообщений, `/start` обрабатывается по умолчанию
...
Б) Reply-клавиатура для часто используемых действий
В) Стандартный текстовый ввод без дополнительных элементов
Г) Inline-кнопки для контекстных ответов
...
А) Использовать `ReplyKeyboardMarkup` и обработать выбор через `@bot.message_handler()`
Б) Сохранить идентификатор подписки в глобальной переменной и обработать нажатие в `@bot.message_handler()`
В) Передать идентификатор подписки через параметр `message.text` и обработать в обработчике текстовых сообщений
Г) Получить `callback_query` и прочитать `callback_data` через `@bot.callback_query_handler`
...
А) Использовать временные переменные внутри обработчика без сохранения между запросами
