...
class TestOR11_HTTPRequest:
    ...
    def test_get_request_formed_correctly(self, mock_requests, sample_quote_data):
        ...
        assert "quotable.io" in call_args[0][0] or "tags" in str(call_args)


class TestOR12_StatusCodeHandling:
    """ОР 1.2: Анализировать ответ сервера, интерпретировать статус-коды"""
    
    def test_status_200_returns_data(self, mock_requests, sample_quote_data):
...
class TestOR21_RequestsUsage:
    ...
    def test_uses_requests_get(self, mock_requests, sample_quote_data):
...
class TestOR23_JSONParsing:
    ...
    def test_json_response_parsed(self, mock_requests, sample_quote_data):
        ...
        assert result is not None
        assert isinstance(result, dict)
        assert result["author"] == "Walt Disney"
