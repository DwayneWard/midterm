...
class TestOR61_ErrorHandling:
    ...
    def test_500_handler_exists(self, client):
        ...
        response = client.get("/recipes/invalid-id")
...
class TestOR62_HTTPStatusCodes:
    ...
    def test_get_existing_recipe_returns_200(self, client):
    ...
    def test_create_recipe_returns_201(self, client, sample_recipe_create):
        """Проверка статуса 201 для создания ресурса"""
        response = client.post("/api/recipes", json=sample_recipe_create)
        if response.status_code == 201:
            assert response.status_code == 201
    
    def test_delete_recipe_returns_204(self, client):
        ...
        create_response = client.post("/api/recipes", json=new_recipe)
