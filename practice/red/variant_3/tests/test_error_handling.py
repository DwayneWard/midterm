...
class TestOR61_ErrorHandling:
    ...
    def test_500_handler_exists(self, client):
        ...
        response = client.get("/events/invalid-id")
...
class TestOR62_HTTPStatusCodes:
    ...
    def test_get_existing_event_returns_200(self, client):
    ...
    def test_create_event_returns_201(self, client, sample_event_create):
        """Проверка статуса 201 для создания ресурса"""
        response = client.post("/api/events", json=sample_event_create)
        if response.status_code == 201:
            assert response.status_code == 201
    
    def test_delete_event_returns_204(self, client):
