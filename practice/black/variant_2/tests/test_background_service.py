...
class TestOR42_APScheduler:
    ...
    @pytest.mark.asyncio
    @patch('storage.get_all_products')
    @patch('storage.delete_product')
    async def test_cleanup_old_products(self, mock_delete, mock_get_all):
        """Проверка функции очистки старых продуктов"""
        try:
            from models import Product
            from datetime import datetime, timedelta
            
            # Создаем старый продукт с нулевым остатком
            old_product = Product(
                id=1,
                name="Old product",
                price=99.99,
            ...
            mock_get_all.return_value = [old_product]
            mock_delete.return_value = True
            
            await cleanup_old_products()
            
            # Проверяем, что delete_product был вызван (если функция реализована)
