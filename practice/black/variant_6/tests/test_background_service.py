...
class TestOR42_APScheduler:
    ...
    @pytest.mark.asyncio
    @patch('storage.get_all_articles')
    @patch('storage.delete_article')
    async def test_cleanup_unpublished_articles(self, mock_delete, mock_get_all):
        """Проверка функции очистки неопубликованных статей"""
        try:
            from models import Article
            from datetime import datetime, timedelta
            
            # Создаем старую неопубликованную статью
            old_article = Article(
                id=1,
                title="Old Article",
                content="Content",
            ...
            mock_get_all.return_value = [old_article]
            mock_delete.return_value = True
            
            await cleanup_unpublished_articles()
            
            # Проверяем, что delete_article был вызван (если функция реализована)
