...
class TestOR42_APScheduler:
    ...
    @pytest.mark.asyncio
    @patch('storage.get_all_tasks')
    @patch('storage.delete_task')
    async def test_cleanup_old_tasks(self, mock_delete, mock_get_all):
        """Проверка функции очистки старых задач"""
        try:
            from models import Task
            from datetime import datetime, timedelta
            
            # Создаем старую выполненную задачу
            old_task = Task(
                id=1,
                title="Old task",
                priority="low",
            ...
            mock_get_all.return_value = [old_task]
            mock_delete.return_value = True
            
            await cleanup_old_tasks()
            
            # Проверяем, что delete_task был вызван (если функция реализована)
